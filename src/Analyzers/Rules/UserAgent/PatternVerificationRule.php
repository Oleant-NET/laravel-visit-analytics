...
namespace Oleant\VisitAnalytics\Analyzers\Rules\UserAgent;
...
class PatternVerificationRule implements RuleInterface
{
     ...
     * @param array $params Configuration: 'ua_regex_patterns', 'weights', 'os_mapping'.
     * @return void
     */
        ...
        foreach ($patterns as $key => $data) {
            ...
            if (@preg_match($delimiter, $ua)) {
                ...
                if ($needsVerification) {
                    ...
                    $state->add($weight, "ua_match_{$key}", [
                        'matched_pattern' => $pattern
                    ]);
                }
            }
