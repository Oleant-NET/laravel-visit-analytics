...
class UserAgentStabilityRule extends AbstractBehaviorRule
{
    /**
     * Detects if the User-Agent changes for the same IP within a short window.
     * 
     * Real users rarely switch browsers or devices mid-session. Rapid
     * UA switching is a high-confidence indicator of scripted activity.
     *
     * @param VisitLog $log
    ...
    public function apply(VisitLog $log, AnalysisState $state, array $params): void
    {
        ...
        if ($differentUA) {
            $state->add(
                (int)($params['weights']['ua_change_anomaly'] ?? 40),
                'ua_change_anomaly',
                [
